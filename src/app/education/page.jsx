...
export default function EducationPage() {
    ...
    <div className="mt-20 p-4 max-w-[1250px] mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {classes.map((item, index) => (
          <div key={index} className="rounded-xl p-6 shadow-md hover:shadow-xl transition-all duration-300 bg-color">
  ...
  <div className="mt-3">
    ...
    <ul className="list-disc list-inside text-gray-700 dark:text-gray-400 mt-1">
      {item.teachers?.map((teacher, i) => (
        <li key={i}>🎓 {teacher.name} - <span className="text-sm text-gray-600 dark:text-gray-400">{teacher.subject}</span></li>
      ))}
    </ul>
  </div>

  <div className="mt-3">
    <strong className="text-indigo-600">📅 Days:</strong> <span className="text-gray-800 dark:text-gray-400">{item.days?.join(', ')}</span>
  </div>

  {item.notes && (
    <div className="mt-3 text-sm text-gray-700 dark:text-gray-400 border-t pt-2">
      <strong className="text-indigo-600">📝 Notes:</strong> {item.notes}
    </div>
  )}

  {/* Fees Section */}
  <div className="mt-3 text-gray-800 dark:text-gray-400 space-y-1">
